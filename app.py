...
if data is not None:
    ...
    def plot_stock_with_all_signals(data, symbol, volume_profile=None):
        ...
        try:
            # --- RESET STYLES AND SET BASE FONT SIZES ---
            ...
            try:
                ...
                st.warning(f"Shares outstanding error: {e}. Using relative volume.", icon="⚠️")
                shares_outstanding = None
                valid_volume = data['Volume'].dropna()
                if not valid_volume.empty and valid_volume.mean() != 0: data['VolumeRatio'] = data['Volume'].rolling(window=30, min_periods=1).mean() / valid_volume.mean()
                else: data['VolumeRatio'] = 0
            ...
            def calculate_rsi_divergence(close, rsi, lookback, price_thresh, rsi_thresh):
                ...
                for i in range(lookback, len(close)):
                    ...
                    price_lower = close.iloc[i] < close.iloc[i - lookback]; rsi_higher = rsi.iloc[i] > rsi.iloc[i - lookback]
                    price_higher = close.iloc[i] > close.iloc[i - lookback]; rsi_lower = rsi.iloc[i] < rsi.iloc[i - lookback]
            ...
            fig, (ax1, ax_drawdown, ax_up_down, ax2, ax3) = plt.subplots(
                5, 1, figsize=(14, 22), sharex=True, # Keep slightly taller figure
                gridspec_kw={'height_ratios': [5, 1.5, 1.5, 1.5, 1.5]}
            )
            # No plt.style.use() here - defaults to white background
            ...
            # *** CHANGE: SMA200 linestyle to solid ('-') ***
            ax1.plot(data.index, data['SMA200'], label='SMA 200', color='red', linestyle='-', linewidth=1.3) # Solid line, slightly thicker
            ...
            ax1.grid(True) # Enable grid explicitly

            # Highlight Main Signal (Keep improvements from previous step)
            ...
            if volume_profile is not None and not volume_profile.empty:
                ...
                ax1v.set_xlabel("Vol Profile", color='purple', alpha=0.8, fontsize=11)
            ...
            ax1_2.set_ylabel('Volume Ratio', color='dimgray', alpha=0.9, fontsize=12)
            ax1_2.tick_params(axis='y', labelcolor='dimgray', colors='dimgray', labelsize=11)
            ...
            ax_drawdown.set_ylabel('Drawdown', color='cornflowerblue', fontsize=12)
            ax_drawdown.tick_params(axis='y', labelcolor='cornflowerblue', labelsize=11)
            ax_drawdown.yaxis.set_major_formatter(mtick.PercentFormatter(xmax=1.0))
            ax_drawdown.grid(True) # Enable grid explicitly
            ax_drawdown.set_title('Price Drawdown from Peak', loc='left', fontsize=12, alpha=0.9)
            ...
            ax_drawdown.text(last_date, last_drawdown_value, f' {last_drawdown_value:.1%}', verticalalignment='center', horizontalalignment='left', fontsize=11, color='red')
            ...
            ax_up_down.set_ylabel('Up/Down Vol', color='darkorange', fontsize=12)
            ax_up_down.tick_params(axis='y', labelcolor='darkorange', labelsize=11)
            ax_up_down.grid(True) # Enable grid explicitly
            ax_up_down.set_title('Up/Down Volume Momentum', loc='left', fontsize=12, alpha=0.9)
            ...
            ax2.set_ylim(0, 100)
            ax2.grid(True) # Enable grid explicitly
            ax2.set_title('Relative Strength Index (RSI)', loc='left', fontsize=12, alpha=0.9)
            ...
            # Calculate dynamic width based on number of days displayed
            num_days = (data.index.max() - data.index.min()).days
            bar_width = max(0.5, min(2.0, num_days / 252.0 * 0.8 if num_days > 0 else 1.0)) # Heuristic width
            ...
            ax3.set_ylabel('MACD', color='blue', fontsize=12)
            ax3.tick_params(axis='y', labelcolor='blue', labelsize=11)
            ax3.tick_params(axis='x', labelsize=11)
            ax3.grid(True) # Enable grid explicitly
            ax3.set_title('MACD (20, 40, 20)', loc='left', fontsize=12, alpha=0.9)
            ...
            st.pyplot(fig)
            plt.close(fig)
            # plt.rcdefaults() # Already called at the beginning, and in finally


            # --- [Combined Signal Table remains the same] ---
            data['MainSignal'] = data['Signal'].astype(int)
            data['PositiveRSI'] = data['PositiveRSIDivergence'].astype(int); data['NegativeRSI'] = data['NegativeRSIDivergence'].astype(int)
            data['PositiveMACD'] = data['PositiveMACDSignal'].astype(int); data['NegativeMACD'] = data['NegativeMACDSignal'].astype(int)
            ...
            signal_cols_display = ['Close'] + signal_cols_to_check
            signal_active_mask = data[signal_cols_to_check].any(axis=1)
            signal_data = data.loc[signal_active_mask, signal_cols_display].tail(15).copy()
            ...
            if not signal_data.empty:
                signal_data.reset_index(inplace=True); signal_data.rename(columns={'index': 'Date', 'Close': 'Price'}, inplace=True)
                signal_data['Date'] = pd.to_datetime(signal_data['Date']).dt.strftime('%Y-%m-%d'); signal_data['Ticker'] = symbol
                ...
                display_order = [col for col in display_order if col in signal_data.columns]; signal_data = signal_data[display_order]
                signal_data = signal_data.sort_values(by='Date', ascending=False)
                st.write(f"\nLast {len(signal_data)} Signal Occurrences:")
                styled_df = signal_data.style.apply(highlight_signals, axis=1).format({'Price': '${:,.2f}'})
