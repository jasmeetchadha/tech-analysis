...
def plot_stock_with_all_signals(symbol, period):
    """
    Downloads stock data, calculates signals, and plots:
    ...
    try:
        # Download data
        data = yf.download(tickers=symbol, period=period, group_by='Ticker')
        if data.empty:
            st.error(f"No data found for {symbol}")
            return None, None

        if symbol not in data.columns:
            st.error(f"No data found for {symbol} for this period.")
            return None, None
        ...
        plt.tight_layout()
        #plt.show() #Don't show, return fig
...
period = st.selectbox("Select Time Period:", ("1mo", "3mo", "6mo", "1y", "2y", "5y", "10y", "ytd", "max"))

if ticker:
    fig, signal_table = plot_stock_with_all_signals(ticker, period)

    if fig:
        st.pyplot(fig)

    if signal_table is not None and not signal_table.empty:
        st.subheader("Last 10 Signals (All Types):")
        st.dataframe(signal_table)
    elif signal_table is not None:
        st.write("No signals generated for the selected period.")
