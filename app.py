    ...
    def plot_stock_with_all_signals(data, symbol, volume_profile=None):
        ...
        try:
            ...
            fig, (ax1, ax_drawdown, ax_up_down, ax2, ax3) = plt.subplots(
                5, 1, figsize=(15, 24), sharex=True, # Kept slightly larger figure
            ...
            # *** ADD X-LABEL AND TICKS TO TOP PANEL (ax1) ***
            ax1.set_xlabel('Date')
            ax1.tick_params(axis='x', labelbottom=True) # Force tick labels on top x-axis

            # Main Signal Highlighting (axvspan loop)
            main_signal_dates = data.index[data['Signal'] == 1]
            if not main_signal_dates.empty:
                ax1.plot([], [], color='lightgreen', linewidth=10, label='Main Signal', alpha=0.6) # Legend entry
                for date in main_signal_dates:
                    start_date = date - pd.Timedelta(days=0.5); end_date = date + pd.Timedelta(days=0.5)
                    ax1.axvspan(start_date, end_date, color='lightgreen', alpha=0.4, zorder=1)
            ...
            if volume_profile is not None and not volume_profile.empty:
                ...
                if price_max > price_min: plot_width = (price_max - price_min) * (volume_profile_width / 100.0); bin_height = ((price_max - price_min) / num_bins * 0.8) if num_bins > 0 else 1
                else: plot_width = 1; bin_height = 1
            ...
            else: ax1.text(0.02, 0.95, "Vol Profile Unavailable", transform=ax1.transAxes, color='red', alpha=0.7, ha='left', va='top', fontsize=11)
            ...
            # Combine legends
            lines1, labels1 = ax1.get_legend_handles_labels(); lines1_2, labels1_2 = ax1_2.get_legend_handles_labels()
            valid_lines1 = [l for l, lbl in zip(lines1, labels1) if lbl != '_nolegend_']; valid_labels1 = [lbl for l, lbl in zip(lines1, labels1) if lbl != '_nolegend_']
            valid_lines1_2 = [l for l, lbl in zip(lines1_2, labels1_2) if lbl != '_nolegend_']; valid_labels1_2 = [lbl for l, lbl in zip(lines1_2, labels1_2) if lbl != '_nolegend_']
            if valid_lines1 or valid_lines1_2: ax1.legend(valid_lines1 + valid_lines1_2, valid_labels1 + valid_labels1_2, loc='upper left')
            ...
            # -- Panel 3: Up/Down Volume Plot (Markers) --
            ...
            ax_up_down.axhline(0.5, color='green', linestyle='--', linewidth=1, label='+0.5 Threshold'); ax_up_down.axhline(-0.5, color='red', linestyle='--', linewidth=1, label='-0.5 Threshold'); ax_up_down.axhline(0, color='black', linestyle='-', linewidth=0.6, alpha=0.6)
            ax_up_down.set_ylabel('Up/Down Vol', color='darkorange'); ax_up_down.tick_params(axis='y', labelcolor='darkorange'); ax_up_down.grid(True); ax_up_down.set_title('Up/Down Volume Momentum', loc='left', alpha=0.9)
            pos_udv_dates = data.index[data['PositiveUpDownVolumeSignal'] == 1]; neg_udv_dates = data.index[data['NegativeUpDownVolumeSignal'] == 1]; udv_plots = []
            if not pos_udv_dates.empty: p1, = ax_up_down.plot(pos_udv_dates, data.loc[pos_udv_dates, 'UpDownVolume'], 'go', markersize=6, alpha=0.8, label='Pos Signal', linestyle='None'); udv_plots.append(p1)
            if not neg_udv_dates.empty: p2, = ax_up_down.plot(neg_udv_dates, data.loc[neg_udv_dates, 'UpDownVolume'], 'ro', markersize=6, alpha=0.8, label='Neg Signal', linestyle='None'); udv_plots.append(p2)
            line_handles, line_labels = ax_up_down.get_legend_handles_labels(); marker_labels = [p.get_label() for p in udv_plots]; final_handles = line_handles + [p for p, label in zip(udv_plots, marker_labels) if label not in line_labels]; final_labels = line_labels + [label for label in marker_labels if label not in line_labels];
            if final_handles: ax_up_down.legend(handles=final_handles, labels=final_labels)

            # -- Panel 4: RSI Plot --
            ax2.plot(data.index, data['RSI'], label='RSI (14)', color='purple', linewidth=1.2); ax2.axhline(70, color='red', linestyle='--', linewidth=1, label='Overbought (70)'); ax2.axhline(50, color='gray', linestyle=':', linewidth=0.8, alpha=0.7); ax2.axhline(30, color='green', linestyle='--', linewidth=1, label='Oversold (30)'); ax2.set_ylabel('RSI', color='purple'); ax2.tick_params(axis='y', labelcolor='purple'); ax2.set_ylim(0, 100); ax2.grid(True); ax2.set_title('Relative Strength Index (RSI)', loc='left', alpha=0.9); pos_rsi_div_dates = data.index[data['PositiveRSIDivergence']]; neg_rsi_div_dates = data.index[data['NegativeRSIDivergence']]; rsi_plots = []
