...
if data is not None:
    ...
    def plot_stock_with_all_signals(data, symbol, volume_profile=None):
        ...
        try:
            ...
            # Main Signal Highlighting (axvspan - CORRECTED ITERATION)
            main_signal_active = data['Signal'] == 1
            if main_signal_active.any():
                ax1.plot([], [], color='lightgreen', linewidth=10, label='Main Signal', alpha=0.6) # Legend entry
                # Find start/end points of contiguous blocks
                signal_blocks = main_signal_active.ne(main_signal_active.shift()).cumsum()
                grouped_by_block = data.index[main_signal_active].groupby(signal_blocks[main_signal_active])

                # Iterate safely through the identified groups
                for block_key in grouped_by_block.groups:
                    block_indices = grouped_by_block.get_group(block_key) # Get the DatetimeIndex for this group
                    if not block_indices.empty: # Ensure the block is not empty
                        start = block_indices.min() - pd.Timedelta(days=0.5)
                        end = block_indices.max() + pd.Timedelta(days=0.5)
                        ax1.axvspan(start, end, color='lightgreen', alpha=0.4, zorder=1)
            ...
            # -- Panel 3: Up/Down Volume Plot (RESTORED MARKERS) --
            ...
            if not pos_udv_dates.empty:
                ax_up_down.plot(pos_udv_dates, data.loc[pos_udv_dates, 'UpDownVolume'], 'go', markersize=6, alpha=0.8, label='Pos Signal', linestyle='None')
            if not neg_udv_dates.empty:
                ax_up_down.plot(neg_udv_dates, data.loc[neg_udv_dates, 'UpDownVolume'], 'ro', markersize=6, alpha=0.8, label='Neg Signal', linestyle='None')
            ax_up_down.legend()
            ...
            if not pos_rsi_div_dates.empty: ax2.plot(pos_rsi_div_dates, data.loc[pos_rsi_div_dates, 'RSI'], 'g^', markersize=7, alpha=0.85, label='Pos Div', linestyle='None')
            if not neg_rsi_div_dates.empty: ax2.plot(neg_rsi_div_dates, data.loc[neg_rsi_div_dates, 'RSI'], 'rv', markersize=7, alpha=0.85, label='Neg Div', linestyle='None')
            ax2.legend()
            ...
            if not pos_macd_sig_dates.empty: ax3.plot(pos_macd_sig_dates, data.loc[pos_macd_sig_dates, 'MACD_20_40_20'], 'g^', markersize=7, alpha=0.85, label='Pos Signal', linestyle='None')
            if not neg_macd_sig_dates.empty: ax3.plot(neg_macd_sig_dates, data.loc[neg_macd_sig_dates, 'MACD_20_40_20'], 'rv', markersize=7, alpha=0.85, label='Neg Signal', linestyle='None')
            ax3.legend()
