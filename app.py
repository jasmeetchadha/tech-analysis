...
if data is not None:
    ...
    def plot_stock_with_all_signals(data, symbol, volume_profile=None):
        ...
        try:
            ...
            # Main Signal Highlighting (axvspan)
            ...
            if main_signal_active.any():
                ...
                signal_ranges = data.index[main_signal_active].groupby(signal_blocks[main_signal_active])
                for _, block_indices in signal_ranges:
            ...
            # -- Panel 3: Up/Down Volume Plot (RESTORED) --
            ...
            if not pos_udv_dates.empty:
                ax_up_down.plot(pos_udv_dates, data.loc[pos_udv_dates, 'UpDownVolume'], 'go', markersize=6, alpha=0.8, label='Pos Signal', linestyle='None') # Added linestyle='None'
            if not neg_udv_dates.empty:
                ax_up_down.plot(neg_udv_dates, data.loc[neg_udv_dates, 'UpDownVolume'], 'ro', markersize=6, alpha=0.8, label='Neg Signal', linestyle='None') # Added linestyle='None'
            ax_up_down.legend()
            ...
            if not pos_rsi_div_dates.empty: ax2.plot(pos_rsi_div_dates, data.loc[pos_rsi_div_dates, 'RSI'], 'g^', markersize=7, alpha=0.85, label='Pos Div', linestyle='None') # Added linestyle='None'
            if not neg_rsi_div_dates.empty: ax2.plot(neg_rsi_div_dates, data.loc[neg_rsi_div_dates, 'RSI'], 'rv', markersize=7, alpha=0.85, label='Neg Div', linestyle='None') # Added linestyle='None'
            ax2.legend()
            ...
            if not pos_macd_sig_dates.empty: ax3.plot(pos_macd_sig_dates, data.loc[pos_macd_sig_dates, 'MACD_20_40_20'], 'g^', markersize=7, alpha=0.85, label='Pos Signal', linestyle='None') # Added linestyle='None'
            if not neg_macd_sig_dates.empty: ax3.plot(neg_macd_sig_dates, data.loc[neg_macd_sig_dates, 'MACD_20_40_20'], 'rv', markersize=7, alpha=0.85, label='Neg Signal', linestyle='None') # Added linestyle='None'
            ax3.legend()
